...
(all [name types methods]
     ...
     (all [types' methods']
          ...
          (exist-within [types [types methods]]
                        (= types types')
                        (= methods methods'))))
...
3. A rule for `methods-for-class`, providing the methods defined for this
   class. The base rule for this predicate is provided below.
```clojure
...
Now we can define our first type class. `Showable` is a class with one
method: `show`, which takes a single argument of a member type and returns
...
```
This defines a case for `methods-for-class` and a base rule for predicate
`Show`.
```clojure
(assert
...
type list (we used `x`). A corresponding value will appear in the method
declaration.
