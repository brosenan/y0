...
(all [x]
     (notpeano3 x) <-
     (fail (peano x) ? x "is not a Peano number"
           ! x "is not a proper notpeano because" !?))
(assert
 (notpeano3 0 ! 0 "is not a proper notpeano because" ["Please use z instead of 0"])
 (notpeano3 z ! z "is not a proper notpeano because" [(peano z) "succeeded when it should have failed"])
 (notpeano3 1))
...
(all [a b] (foobar-type (+ a b) foolish) <-
     (exist [type]
            (foobar-type a type)
            (foobar-type b type
                         ! "Type mismatch. Expression" a "is" type "but" b "is not")))
...
(assert
 ...
 (foobar-type (+ foo bar) foolish
              ! "Type mismatch. Expression" foo "is" foolish "but" bar "is not"))
...
As a side-note, the explanation we provided for mis-typed `+` and
`*` is lacking. We knew the type of `a`, but not the type of `b`.

To fix this, let us first introduce the predicate `my=`:
```clojure
(all [x] (my= x x))

```
This predicate simply unifies the values of its two arguments.

Now consider the following definition of operator `**`, a
better-explained definition of `*`:
```clojure
(all [a b] (foobar-type (** a b) barley) <-
     (exist [a-type b-type]
            (foobar-type a a-type)
            (foobar-type b b-type)
            (my= a-type b-type
...
```
The build-in predicate `=` replaces the need to define predicates
such as `my=`, but on that later.

`exist` conditions are useful in tests, as they allow us to
introduce free variable. Recall the test above:
`(foobar-type (** foo bar) barley ! ...)`. In this test we had to
"guess" the result (`barley`) returned by `foobar-type` despite it
