...
(all [fds xs]
     (member-initialization fds xs ! fds "is not a member definition list"))

(all [fd fds xs]
     (member-initialization [fd & fds] xs) <-
     (exist [x xs']
            (= xs [x & xs']
...
(all [xs]
     (member-initialization [] xs) <-
     (= xs [] ! "Too many elements in initializer list." xs "are extra"))

(all [fd t]
     (field-type fd t ! fd "is not a field"))

(all [v t]
     (field-type [:field_def t v] t))
