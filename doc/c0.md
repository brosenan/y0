...
```clojure
(all [t xs]
     (initializer-for-type t xs ! "Cannot use an initializer list for type" t))

(all [alias xs]
     (initializer-for-type [:type_alias alias] xs) <-
     (exist [t]
            (type-alias alias t)
            (initializer-for-type t xs)))

(all [fds xs]
     (initializer-for-type [:struct_type & fds] xs) <-
     (member-initializations xs fds))
...
(all [xs fds]
     (member-initializations xs fds ! xs "is not a member initializer list"))

(all [x xs fds]
     (member-initializations [x & xs] fds) <-
     (exist [fd fds']
            (= fds [fd & fds']
               ! "Too many elements in initializer list." x "is extra")
            (member-initialization-for-field-kind fd x)
            (member-initializations xs fds')))

(all [fds]
     (member-initializations [] fds) <-
     (= fds [] ! "Too few elements in initializer list." fds "are not initalized"))

(all [fd x]
     (member-initialization-for-field-kind fd x ! fd "is not a member definition"))

(all [t v x]
     (member-initialization-for-field-kind [:field_def t v] x) <-
     (expr-of-type x t))
...
(all [n opt-defs x]
     (member-initialization-for-field-kind [:union_def n & opt-defs] x) <-
     (union-member-initialization x n opt-defs))
...
(all [n t xs]
     (initializer-for-type [:array_type n t] xs) <-
     (exist [vn]
...
(all [t xs]
     (initializer-for-type [:slice_type t] xs) <-
     (exist [begin end]
            (= xs [begin end]
