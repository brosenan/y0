...
(all [fd x]
     ...
     (exist [t v]
            (= fd [:field_def t v] 
               ! "Field initializer" x "is inappropriate for field definition" fd)
            (expr-of-type x t)))
...
(all [opt val fd]
     (member-initialization [:option_initializer opt val] fd) <-
     (exist [n opt-defs]
            (= fd [:union_def n & opt-defs]
               ! "Union initialization for non-union field" fd "was given")
