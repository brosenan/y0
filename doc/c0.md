...
```clojure
(all [arr idx t]
     (expr-of-type [:element_access arr idx] t) <-
     (exist [tarr]
            (typeof arr tarr)
            (indexable-type tarr ! "trying to access element of" arr))
     (expr-of-type idx [:uint64_type]))

(all [t]
     (indexable-type t ! t "is not an indexable type"))

(all [n t]
     (indexable-type [:array_type n t]))

(all [a]
     (indexable-type [:type_alias a]) <-
     (exist [t]
            (type-alias a t)
            (indexable-type t)))
```
