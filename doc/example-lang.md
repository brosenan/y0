...
```
The second statement is a rule that accepts calls to this function as $y_1$
expressions. It repeats the same `given` condition as in the assertion, but
this time type `type` is bound to the expression's type.
